...
function getFallbackImage(category) {
  return UNSPLASH_CATEGORIES[category] || UNSPLASH_CATEGORIES["default"];
}
...
function StoryCard({story, onClick, locked, onLock}) {
  ...
  const img = story.articles?.[0]?.image_url || getFallbackImage(story.category);
  ...
  return (
    <div onClick={locked?onLock:()=>onClick(story)}
      ...
      <div style={{padding:"14px 15px 15px"}}>
        ...
        {srcIds.length>0 && (
          <div style={{display:"flex",gap:"4px",marginTop:"11px",flexWrap:"wrap",alignItems:"center"}}>
            {srcIds.slice(0,7).map(id=><SrcChip key={id} id={id} size={24}/>)}
            {srcIds.length>7 && <div style={{width:24,height:24,borderRadius:"5px",background:"#1f1f1f",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"9px",color:"#333"}}>+{srcIds.length-7}</div>}
...
function StoryModal({story, onClose}) {
  ...
  const cov = story.coverageByOrientation||story.coverage_by_orientation||{};
  const img = articles[0]?.image_url||getFallbackImage(story.category);
  const catIcon = CAT_ICONS[story.category]||CAT_ICONS["default"];
